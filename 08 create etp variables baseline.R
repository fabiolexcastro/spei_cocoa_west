...
# Baseline data
crnt <- dir_ls('./data/raster/terraclimate/baseline') 
crnt <- as.character(crnt)
crnt <- mixedsort(crnt)

tmax <- grep('tmax', crnt, value = TRUE)
tmin <- grep('tmin', crnt, value = TRUE)
prec <- grep('ppt', crnt, value = TRUE)
...
srad <- raster::resample(srad, raster::stack(crnt[1]), method = 'bilinear')
